...
# names and associated port numbers.
namesAndPorts = {
    'A': 8000,
    'B': 8001,
    'C': 8002,
...
def CreateTCPPacket(sourceID, destinationID, acknowledgementNumber, sequenceNumber, data, urgentPointer, synBit, finBit,
                    rstBit, terBit):
    ...
    headerLength = len(str(sourceID)) + len(str(destinationID)) + len(str(acknowledgementNumber)) + len(
        str(sequenceNumber)) + len(str(urgentPointer)) + len(str(synBit)) + len(str(finBit)) + len(str(rstBit)) + len(
        str(terBit))
